<?php
    ...
    if (isset($_GET['id'])) 
    {
        ...
        $stmt = $pdo->prepare("SELECT id, title, category, images, content,status FROM blogs WHERE id = ?");
        $stmt->execute([$postId]);
    ...
    if(isset($_GET['did']))
    {
        ...
        $stmt->execute([$postId]);
        header('Location: blog-adminpanel.php');
        exit();
    }
?>
<!DOCTYPE html>
...
<div class="bgded top"> 
    ...
    <section id="navwrapper" class="hoc clear"> 
        <nav id="mainav">
            <ul class="clear">
                ...
                <li><a href="index.php?log=1">Logout Admin</a></li>
    ...
    <div class="post-container">
        <div class="post-image">
            ...
            <a href="addpost.php?eid=<?php echo $post['id']; ?>"><input type="button" class="share-toggle" value="Edit"/></a>
            <a href="blog-edit.php?id=<?php echo $post['id']; ?>&did=<?php echo $post['id']; ?>"><input type="button" class="share-toggle" value="Delete"/></a>
            <?php
            if ($post['status'] == 0)
            {
                ?><a href="blog-edit.php?id=<?php echo $post['id']; ?>&status=1"><input type="button" class="share-toggle" value="Approve"/></a><?php
            }
            ?>
        </div>
